...
- **AI試合振り返り** — 対戦ログ、採用統計、相性、ダメージ計算、局面解析を参照しながら、Gemini / OpenAI / Anthropic と会話して一戦を振り返れます。利用するプロバイダーのAPIキーが必要で、API利用料が発生する場合があります
...
- **入力**: SwitchおよびAndroid / iPhoneの横向き動画。解像度・画面比率は自動判定
- **推奨録画設定**: 1920×1080 / 30fps。1280×720などを含め、16:9のゲーム画面が最も安定します。低解像度では文字の認識精度が下がる場合があります
- **形式**: ポケモンチャンピオンズのシングル / ダブルバトル
- **Live Scan**: OBS Virtual Camera または同等のカメラ入力
- **ストレージ**: アプリ本体と初回起動時にダウンロードされるモデルファイルで数百 MB 程度

スマートフォンのほかの横向き比率にも対応しますが、画面配置によって認識精度が下がる場合があります。ゲーム画面を引き伸ばしたり切り抜いたりせず、余白なく録画してください。
